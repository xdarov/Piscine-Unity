...
public class DragAndDropItem : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
	...
	public void OnBeginDrag(PointerEventData eventData)
	{
		if (dragDisabled == false)
		{
            ...
            RawImage myRawImage = GetComponent<RawImage>();
            if(myRawImage)
            {
                    myRawImage.raycastTarget = false;                                          // Disable icon's raycast for correct drop handling
                    RawImage iconRawImage = icon.AddComponent<RawImage>();
                    iconRawImage.raycastTarget = false;
                iconRawImage.texture = myRawImage.texture;
            }
            else 
            {
	...
	public void OnEndDrag(PointerEventData eventData)
	{
        ...
        if (GameManager.gm.dropping && !isSkill)
        {
            gameObject.transform.position = GameManager.gm.Player.transform.position;
	...
	public void MakeRaycast(bool condition)
	{
        ...
        else
        {
            ...
            if (rawImage != null)
            {
                rawImage.raycastTarget = condition;
            }
        }
	}
